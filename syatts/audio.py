...
def wav_to_logmel(path: str, cfg: dict) -> torch.Tensor:
    ...
    return torch.log(torch.clamp(mel, min=cfg.get("mel_floor", 1e-5)))


def logmel_to_wav(logmel: torch.Tensor, cfg: dict) -> torch.Tensor:
    logmel = torch.clamp(logmel, min=math.log(cfg.get("mel_floor", 1e-5)), max=8.0)
    mel = logmel.exp().cpu()
    ...
    griffin = torchaudio.transforms.GriffinLim(
        ...
        win_length=cfg["win_length"],
        hop_length=cfg["hop_length"],
        n_iter=cfg.get("griffin_lim_iters", 64),
    )
